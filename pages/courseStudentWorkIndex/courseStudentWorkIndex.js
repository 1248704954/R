...
Page({
    ...
    this.editorCtx.getContents({
      success: (res) => {
        ...
        wx.cloud.callFunction({
          name: "CourseStudentWorkAdd",
          data: {
            courseID: 1,
            Student_Id: app.globalData.account,
            Assignment_Number:2,
            Assignment_Type: "1",
            Assignment_Condition: app.globalData.html,
          },
          success(res) {
            wx.showToast({
              title: '提交成功',
  ...
  //跳转 courseStudentWorkPreview页面
  toPageCourseStudentWorkPreview() {
    this.editorCtx.getContents({
      success: (res) => {
        console.log(res.html)
        app.globalData.html = res.html
        wx.navigateTo({
          url: '../courseStudentWorkPreview/courseStudentWorkPreview'
        })
